...
def count_status_values(data):
    ...
    groups = {
        'FIR': ['FIR Registered'],
        'FAD': ['Closed', 'Rejected', 'Withdrawal'],
        'CSR': ['NC Registered'],
...
# Sort district_counts based on 'Amount Lost' in descending order
district_counts.sort(key=lambda x: x[4], reverse=True)

# Add 'Rank' column
rank = 1
for district_info in district_counts:
    district_info += (rank,)
    rank += 1
...
<body>
    <div class="container">
        <h1>DISTRICT-WISE RANKED REPORT</h1>
        <h4>January 2024</h4>
        <table>
            <thead>
                <tr>
...
with open("RANKED REPORT.html", "w") as html_file:
    html_file.write(html_content)

print("\n District-wise ranked report generated successfully ! ")
